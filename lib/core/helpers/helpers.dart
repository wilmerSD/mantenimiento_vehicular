...
/*
  ...
  */
  static attachmentFormat({dynamic response, String? filename}) {
  ...
  static int compareTimes(String time1, String time2) {
    ...
    int hour1 = int.parse(parts1[0]) * 60;
    int minute1 = int.parse(parts1[1]);
    int totalMinutes1 = hour1 + minute1;

    int hour2 = int.parse(parts2[0]) * 60;
    int minute2 = int.parse(parts2[1]);
    int totalMinutes2 = hour2 + minute2;
  ...
  Color getCircleColor(int idValidacion) {
    ...
    switch (idValidacion) {
      ...
      case 4:
        ...
        break;
      case 5:
        circleColor = AppColors.validationOutOfTime;
        break;
      case 6:
        circleColor = AppColors.validationOvertime;
        break;
      default:
        circleColor = AppColors.validationDefault;
        break;
    }
    return circleColor;
  }
  ...
  static bool getObscureProperty(dynamic value) {
    if (value == null) {
      return true;
    } else {
      return !value;
  ...
  static bool getObscureProperty2(dynamic value) {
    if (value == null) {
      return true;
    } else {
      return value;
  ...
  /* 📌 Formato del calendario yyyy/MM/dd */
  ...
  /* 📌 Cambiar estado */
  static String swithState(bool value, String? letter) {
    if (value) {
      return "Aprobad$letter";
    } else {
      return "Rechazad$letter";
    }
  }
  ...
  /* 📌 Para validar solo el tipo de entrada (el campo no es requerido obligatoriamente)*/
  static String? simplerequiredRegex(
      String? value, RegExp regExp, String message) {
    if (value == null || value.isEmpty) {
      return null;
    } else if (!regExp.hasMatch(value)) {
      return message;
  ...
  /* 📌 Validar formato */
  static String? requiredRegex(String? value, RegExp regExp, String message) {
    if (value!.isEmpty) {
      return "Este campo es obligatorio";
    } else if (!regExp.hasMatch(value)) {
      return message;
  ...
  static String? comparePassword(String firstPass, String secondPass) {
    if (firstPass != secondPass) {
      return "Asegúrate de que las contraseñas\n sean idénticas";
    } else {
      return null;
    }
  }

  /* 📌 Validar input obligatorio */
  static int? requiredInput(String? value) {
    if (value!.isEmpty) {
      return 1;
    } else {
      return 0;
  ...
  String changeDateToddMMyyyy(String date) {
  ...
  /* 📌 Cambiar formato de fecha de  dd/MM/yyyy  a yyyy/MM/dd */
  static DateTime stringToDateTimeAMD(String dateTimeString) {
    try {
      final formatter = DateFormat('yyyy/MM/dd');
      return formatter.parse(dateTimeString);
    } catch (e) {
  ...
  // 📌 Comparar si dos fechas en formato dd/MM/yyyy son iguales
